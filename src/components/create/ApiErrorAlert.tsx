...
import { AlertTriangle, Info, Wifi, RefreshCw, ArrowLeft } from 'lucide-react';
...
export const ApiErrorAlert: React.FC<ApiErrorAlertProps> = ({ 
  ...
  if (errorMessage.includes('response_format') || errorMessage.includes('unknown_parameter')) {
    ...
    helpText = helpText || "Our team has been notified and is working to fix this. Please try again later.";
  } else if (errorMessage.includes('insufficient_quota')) {
    icon = <Info className="h-5 w-5 text-amber-500" />;
    title = "Service Temporarily Unavailable";
    description = "Our image generation quota has been reached.";
    helpText = helpText || "The service will be available again soon. Please try again later.";
    ...
    helpText = helpText || "Please check your connection and try again.";
  } else if (errorMessage.includes('billing_hard_limit')) {
    icon = <Info className="h-5 w-5 text-amber-500" />;
    title = "Service Temporarily Unavailable";
    description = "Our image generation service has reached its quota limit.";
    helpText = helpText || "The service will be available again soon. Please try again later.";
  }
