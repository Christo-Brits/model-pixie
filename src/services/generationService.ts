    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // Use a properly typed approach to call the RPC function with both input and output types
          const { data: columnCheckData, error: columnCheckError } = await supabase.rpc<boolean, {
            table_name: string;
            column_name: string;
          }>('check_column_exists', {
            table_name: 'jobs',
